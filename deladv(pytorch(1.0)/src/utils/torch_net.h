...
class Torch_net {
public:
	Torch_net(int r, int c):rows(r),cols(c){}
	~Torch_net(){}

	void initNet(std::string model_path) {
		model = torch::jit::load(model_path);
		//model.eval();	
	}

	int forward(std::vector<float> input, std::vector<float> &out) {

		torch::Tensor tensor = torch::from_blob(input.data(), { 1, 1, rows, cols}, torch::kF32);
		std::vector<torch::jit::IValue> in;
		in.push_back(tensor);
		std::clock_t begin = clock();
		torch::Tensor prob = model.forward(in).toTensor();
		std::cout << prob.slice(/*dim=*/1, /*start=*/0, /*end=*/5) << '\n';		
		prob = prob.view({ prob.size(0), -1 });		
		printf(" size is  %d, ans is %f \n", prob.size(0), prob[0].item());
		printf("libtorch time = %d \n", clock() - begin);
		out.insert(out.end(), prob.data<float>(), prob.data<float>() + prob.size(1));
		return 1;
	}
private:
