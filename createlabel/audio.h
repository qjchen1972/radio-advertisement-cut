...
public:
	...
	int mp3_to_wav(const char *input_file_name, const char *output_file_name, float start = 0.0f, float end = 0.0f) {
		...
		int wav_len = end_point - start_point;
		if (wav_len <= 0) return 0;
		//printf("byte_rate = %d  %d  %d\n", byte_rate, wav_len, len);
...
#pragma omp parallel for
	...
	//合成
	int adjust_tone_wav(const char* srcfile, const char* dstfile, float tone) {


		std::vector<int8_t> wav;
		wav_head_t head;

		if (!wav2vec(srcfile, wav, head)) return 0;

		int len = wav.size();
		std::vector<int16_t> ret(len >> 1);
#pragma omp parallel for
		for (int i = 0; i < ret.size(); i++) {
			int temp = round((int)*((int16_t*)(wav.data() + 2 * i))  * tone);
			//printf("%d,", temp);
			if (temp > SHRT_MAX)
				ret[i] = SHRT_MAX;
		...
		if (!vec2wav((int8_t*)ret.data(), ret.size() * 2, dstfile, head.bit_per_sample, head.sr, head.chn))
			return 0;
		return 1;
	}

	//减低采样率转换,单声道
	int wav_down_sr(const char* srcfile, const char* dstfile, int32_t sample_rate) {
		...
		tool.char2GTYPE(wav, fdata, head.bit_per_sample);
		int len = fdata.size() / head.sr  * sample_rate + int(1.0f* (fdata.size() % head.sr)* sample_rate / head.sr);
		std::vector<GTYPE> data(len);
		
		int num = fdata.size() / head.sr;
		int i;
		for (i = 0; i < num; i++) {
			tool.rand_list(fdata.data() + i* head.sr, head.sr, data.data() + i* sample_rate, sample_rate);
		}
		int srclen = fdata.size() - i * head.sr;
		int dstlen = data.size() - i * sample_rate;
		tool.rand_list(fdata.data() + i * head.sr, srclen, data.data() + i * sample_rate, dstlen);
		tool.GTYPE2char(data, wav, head.bit_per_sample);
		if (!vec2wav(wav.data(), wav.size(), dstfile, head.bit_per_sample, sample_rate, head.chn))
			return 0;
		return 1;
	}
